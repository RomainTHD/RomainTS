...
function _visit() {
  _visit = (0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* (node, env) {
    const isLocal = env.getData("isLocal", true);
    const isMutable = env.getData("isMutable", true);
    const eId = yield env.withChildData({
    ...
    (0,_utils__WEBPACK_IMPORTED_MODULE_3__.assert)(eId.identifier !== undefined, "identifier is undefined");
    ...
    if (!node.initializer) {
      // `for (const e of t) ...`
      const externalType = env.getData("varDeclType", true, null);
      if (externalType) {
        vType = externalType;
      } else {
        if (!isMutable) {
      ...
      let e;
      if (vType) {
        e = yield env.withChildData({
          varDeclType: vType
        }, /*#__PURE__*/(0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* () {
          return yield ___WEBPACK_IMPORTED_MODULE_1__.TypeChecker.accept(node.initializer, env);
        }));
      } else {
        e = yield ___WEBPACK_IMPORTED_MODULE_1__.TypeChecker.accept(node.initializer, env);
      }
      ...
      } else if (isMutable && exprType instanceof _types__WEBPACK_IMPORTED_MODULE_2__.LiteralType) {
    ...
    env.add(name, {
      ...
      isLocal,
      isMutable
    }, env.getData("isExported", false, false));
  });
  return _visit.apply(this, arguments);
}
