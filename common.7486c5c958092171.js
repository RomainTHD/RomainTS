...
function _visitFunction() {
  _visitFunction = (0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* (env, nodeParams, nodeRetType) {
    ...
    for (const param of nodeParams) {
      ...
      }, () => _accept__WEBPACK_IMPORTED_MODULE_2__.TypeChecker.accept(param.name, env));
      let pType;
      if (param.type) {
        pType = yield _accept__WEBPACK_IMPORTED_MODULE_2__.TypeChecker.accept(param.type, env);
      } else {
    ...
    let infer = false;
    if (nodeRetType) {
      retType = yield _accept__WEBPACK_IMPORTED_MODULE_2__.TypeChecker.accept(nodeRetType, env);
    } else {
