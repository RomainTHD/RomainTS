...
const visit = /*#__PURE__*/function () {
  var _ref = (0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* (node, env) {
    const expr = yield _accept__WEBPACK_IMPORTED_MODULE_2__.TypeChecker.accept(node.expression, env);
    const prop = yield env.withChildData({
      ...
      return yield _accept__WEBPACK_IMPORTED_MODULE_2__.TypeChecker.accept(node.name, env);
    }));
    if (!(expr instanceof _types__WEBPACK_IMPORTED_MODULE_1__.PropertyAccessor)) {
      throw new _TypecheckingFailure__WEBPACK_IMPORTED_MODULE_3__.TypecheckingFailure(`Property access on non-object type '${expr}'`, node);
    }
    if (expr.hasProperty(prop)) {
      return expr.getProperty(prop);
    }
    if (env.config.strictMode) {
      throw new _TypecheckingFailure__WEBPACK_IMPORTED_MODULE_3__.TypecheckingFailure(`Property '${prop}' does not exist on type '${expr}'`, node);
    } else {
