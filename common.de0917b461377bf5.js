  ...
  _visitUntypedFunction = (0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* (node, env) {
    ...
    if (nodeGenerics) {
      ...
      for (const generic of nodeGenerics) {
        ...
        const constraint = generic.constraint ? yield ___WEBPACK_IMPORTED_MODULE_1__.TypeChecker.accept(generic.constraint, env) : _types__WEBPACK_IMPORTED_MODULE_2__.AnyType.create();
        const defaultType = generic.default ? yield ___WEBPACK_IMPORTED_MODULE_1__.TypeChecker.accept(generic.default, env) : _types__WEBPACK_IMPORTED_MODULE_2__.AnyType.create();
        if (!constraint.contains(defaultType)) {
          throw new ___WEBPACK_IMPORTED_MODULE_1__.TypecheckingFailure(`Default type '${defaultType}' is not assignable to constraint '${constraint}'`, generic);
        }
        seen.add(e.identifier);
        genericsStr.push(e.identifier);
        env.addType(e.identifier, _types__WEBPACK_IMPORTED_MODULE_2__.GenericType.create(e.identifier, defaultType, constraint));
      }
    }
    const params = [];
