...
function _visit() {
  _visit = (0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* (node, env) {
    const {
      fType,
      infer
    } = yield (0,_shared_function__WEBPACK_IMPORTED_MODULE_4__.visitFunction)(env, node.typeParameters, node.parameters, node.type);
    env.enterScope();
    ...
    let retData;
    let bodyData = yield ___WEBPACK_IMPORTED_MODULE_1__.TypeChecker.accept(node.body, env);
