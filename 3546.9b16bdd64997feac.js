...
function _visit() {
  _visit = (0,_home_runner_work_RomainTS_RomainTS_demo_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* (node, env, data) {
    if (node.text.trim() === "") {
      // `x = ;`
    ...
    if (env.getValueSide() === _env__WEBPACK_IMPORTED_MODULE_3__.ValueSide.LValue) {
      ...
      if (!value) {
        if (data?.isPropertyAccess) {
          ...
          throw new _TypecheckingFailure__WEBPACK_IMPORTED_MODULE_4__.TypecheckingFailure(`Identifier '${node.text}' is a type but is incorrectly used as a value`, node);
        } else {
