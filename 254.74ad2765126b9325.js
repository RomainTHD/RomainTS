    ...
    const data = (() => {
      ...
      switch (node.flags) {
        ...
        default:
          ...
          const flags = node.flags.toString(2).split("").reverse().map((x, i) => x === "1" ? i : -1).filter(b => b !== -1).map(b => (typescript__WEBPACK_IMPORTED_MODULE_1___default().NodeFlags)[2 ** b]).join(", ");
          throw new _utils_IllegalStateException__WEBPACK_IMPORTED_MODULE_3__.IllegalStateException(`Unexpected variable declaration list flags: ${flags}`);
      }
    })();
